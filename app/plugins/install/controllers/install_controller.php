...
/**
 * Cette fonction est exécutée avant chaque action du contrôleur.
 * C'est un endroit pratique pour vérifier le statut d'une session
 * ou les permissions d'un utilisateur.
 *
 * @return void
 * @access public
 */
...
/**
 * Cette fonction joue le rôle de protection et empêche l'utilisateur
 * de relancer l'installation si le fichier database.php existe puisque,
 * dans ce cas, l'application est déjà installée.
 *
 * @return void
 * @access protected
 */
    ...
    {
        if (file_exists(CONFIGS . 'databaase.php'))
        {
            $this->Session->setFlash(__('L\'application a déjà été installée !', true));
            $this->redirect('/');
        }
    }
 ...
 */
    ...
    {
        $this->_check();
        $this->set('title_for_layout', __('Vous serez prêt en un clin d\'oeil !', true));
    }

/**
 * Etape 1: Acceptation de la licence
 *
 * La licence du logiciel est présentée à l'utilisateur qui doit la valider
 *
 * @return void
 * @access public
 */
    ...
    {
        $this->_check();
        $this->set('title_for_layout', __('Licence Utilisateur', true));

        if (isset($this->data))
        {
            if (($this->data['Install']['licence']) == 1)
            {
                $this->redirect(array('action' => 'requirement'));
            }
            else
            {
                $this->Session->setFlash('Vous devez accepter la licence pour poursuivre l\'installation !','error');
            }
        }
    }

/**
 * Etape 2: Configuration minimale requise
 *
 ...
 */
    ...
    {
        $this->_check();
        $this->set('title_for_layout', __('Configuration minimale requise', true));
    }
 ...
 */
    ...
    {
        $this->_check();
        $this->set('title_for_layout', __('Connexion à la base de données', true));
        ...
        if (!$db->isConnected())
        {
            $this->Session->setFlash(__('Impossible de se connecter à la base de données.', true), 'default', array('class' => 'erreur message'));
            return;
        }
 ...
 */
    ...
    {
        $this->_check();
        $this->set('title_for_layout', __('Peuplement de la base de données.', true));
 ...
 */
    ...
    {
        $this->set('title_for_layout', __('Fin de l\'installation', true));
